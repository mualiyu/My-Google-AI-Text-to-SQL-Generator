...
The SQL database has three main tables: USERS, PRODUCTS, and ORDERS.

USERS table columns: user_id, username, email, registration_date
PRODUCTS table columns: product_id, name, description, price, stock_quantity, user_id
ORDERS table columns: order_id, user_id, product_id, quantity, order_date, total_price
...
Example 8 - Add new product for John Doe (name: Laptop, ...)
SQL: 
INSERT INTO PRODUCTS (name, price, stock_quantity, user_id)
SELECT 'Laptop', 999.99, 'High-performance laptop', 1, user_id
FROM USERS
...
async def query_database(query: Query):
    sql_query = get_gemini_response(query.question)
    if not sql_query:
